...
public class ManagerDaoImpl implements UserDao {
    @Override
    public User selectAll(){
        ...
        try{
            ...
            //sql操作
            String sql = "select * from qr_manager";
    ...
    public User getpasswordByusername(String username) {
        ...
        try{
            ...
            System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis())+" [server]:数据库已连接");
            //sql操作
            String sql = "select password from qr_manager where username = ?";
            PreparedStatement ps = connection.prepareStatement(sql);
    ...
    public Void register(String username, String password) {
        ...
        try{
            ...
            System.out.println(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis())+" [server]:数据库已连接");
            //sql操作
            String sql = "insert into qr_manager (username , password) value(?,?)";
            PreparedStatement ps = connection.prepareStatement(sql);
            ps.setString(1 , username);
